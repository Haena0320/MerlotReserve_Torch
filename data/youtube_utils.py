...
def download_transcript(id, cache_path):
    ...
    with open(os.path.join(cache_path, f'{id}.v2.info.json'), "w") as f:
        json.dump(info, f)
    return None
...
def download_video_stuff(id,catch_path):
    download_transcript(id,catch_path)
    download_video(id, catch_path)
    return None

def _cleanup(STORAGE_DIR):
    if os.path.exists(STORAGE_DIR):
        shutil.rmtree(STORAGE_DIR)

def upload_blob(source_file, destination_file):
...
if __name__ == "__main__":
    ...
    os.environ[
        "GOOGLE_APPLICATION_CREDENTIALS"] = "/mnt2/user15/merlot_r/merlot_reserve_backup/merlot-343806-11352f466d6d.json"
    sys.path.append('../')

    gclient = storage.Client()
    bucket = gclient.get_bucket("pile_bucket")

    
    video_id_list = []
    with open("/mnt2/user15/merlot_r/merlot_reserve_backup/data/gsdata/yttemporal1b_ids_val.csv") as f:
        reader = csv.DictReader(f)
        for i, row in enumerate(reader):
            video_id_list.append(row["video_id"])     

    num_buffer = 100
    var = len(video_id_list) // num_buffer

    temp_storage = "/mnt2/user15/merlot_r/merlot_reserve_backup/data/youtube_dump/"
    for v in range(var):
        start_ =  time.time()
        try:
            sub_video_id_list = video_id_list[v*num_buffer:(v+1)*num_buffer]
        except:
            print("last file dummy")
            sub_video_id_list = video_id_list[v*num_buffer:]
        
        tasks = []
        for id in sub_video_id_list:
            thread = Thread(target=download_video_stuff, args= (id, f'/mnt2/user15/merlot_r/merlot_reserve_backup/data/youtube_dump/{id}'))
            tasks.append(thread)
            thread.start()
        ...
        for id in sub_video_id_list:
            data = glob.glob(temp_storage+f"{id}/*")

            for d in data:
                try:
                    upload_blob(d, f"youtube_dump/{id}/"+d.split("/")[-1])
                except:
                    continue
            _cleanup(temp_storage+f"{id}")

        print(f"{v*100} data is storaged and the time took {time.time()-start_}")
    
#download_transcript("WAaKRUoY6Io", "/mnt2/user15/merlot_r/merlot_reserve_backup/data/example")

#download_video("WAaKRUoY6Io", "/mnt2/user15/merlot_r/merlot_reserve_backup/data/example")
