...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/mnt2/user15/merlot_r/merlot_reserve_backup/merlot-343806-11352f466d6d.json"
sys.path.append('../')
...
parser.add_argument(
    '-ids_fn',
    dest='ids_fn',
    default="/mnt2/user15/merlot_r/merlot_reserve_backup/data/gsdata/yttemporal1b_ids_val.csv",
    type=str,
...
parser.add_argument(
    '-ids_fn_o',
    dest='ids_fn_o',
    default="/mnt2/user15/merlot_r/merlot_reserve_backup/data/video_log/video_log.csv",
    type=str,
    help='We will use these IDs. you probably should filter them to mkae sure they all at least have the right files. can start with gs://'
)
...
parser.add_argument(
    '-ckpt',
    dest='ckpt',
    default='/mnt2/user15/merlot_r/merlot_reserve_backup/data/gsdata/mobilenetv2_filter_model',
    type=str,
...
# base_fn = "/mnt2/user15/merlot_r/merlot_reserve_backup/data/thepile"
#large_fn = "/mnt2/user15/merlot_r/merlot_reserve_backup/ckpts/large"

# for file_id in tqdm(range(0, 16410)):
#     pile = f"thepile/thepile/fold{file_id:05d}of16410.jsonl.zst"
#     pile_local = os.path.join(base_fn , pile.split("/")[-1])
#     iblob = bucket.blob(pile)
#     iblob.download_to_filename(pile_local)
#iblob = bucket.blob('ckpts/large')
#iblob.download_to_filename(large_fn)
# exit()
...
STORAGE_DIR = "/mnt2/user15/merlot_r/merlot_reserve_backup/data/youtube_dump" #tempfile.mkdtemp()
...
def load_video(video_id):
    ...
    start = time.time()
    try:
        dump_storage_dir = os.path.join(STORAGE_DIR, video_id)
        if not os.path.exists(dump_storage_dir):
            os.mkdir(dump_storage_dir)
        info_fn = os.path.join(dump_storage_dir, '{}.v2.info.json'.format(video_id))
        iblob = bucket.blob('youtube_dump/{}/{}.v2.info.json'.format(video_id, video_id))
        if not iblob.exists():
           return None
        ...
        if 'title' not in item:
            raise ValueError("'title' not in item \n{}".format(item))
        ...
        vtt = pd.DataFrame(item['transcript'], columns=["word", "start", "end"])
        if (vtt.shape[0] == 0) or ('word' not in vtt.columns):
            raise ValueError("'Word' not in item['transcripts'] \n{}".format(item))
        ...
        video_fn = os.path.join(STORAGE_DIR, '{}/{}.mp4'.format(video_id, video_id))
        vblob = bucket.blob('youtube_dump/{}/{}.mp4'.format(video_id, video_id))
        if not vblob.exists():
            try:
                download_video(video_id, os.path.join(STORAGE_DIR, '{}'.format(video_id)))
            except:
                return None
        else:
        ...
        stream_txt = subprocess.run('ffprobe -i {} -show_streams -select_streams a -loglevel error'.format(video_fn),
                                    capture_output=True, shell=True, text=True).stdout
        if len(stream_txt) == 0 or 'codec_type=audio' not in stream_txt:
            print(len(stream_txt))
...
def video_iterator():
    ...
    with tf.io.gfile.GFile(args.ids_fn, 'r') as f:
        ...
        print("loading videos {}".format(len(channels_video_ids)))
    #channels_video_ids = channels_video_ids[949:]
    #channels_video_ids.append("WAaKRUoY6Io")
    if args.shuffle_fns:
        random.shuffle(channels_video_ids)
    print("GOT THE VIDEO IDS - {} in total".format(len(channels_video_ids)))
...
def split_video_into_chunks(item):
    ...
    if args.debug:
        print('duration = {:.3f}. {} audio chunks. #words mean: {:.3f} words max {:2d} words std {:.3f}'.format(
            vtt.iloc[-1]['end'], len(audio_chunks), np.mean(nwords), max(nwords), np.std(nwords)))
...
def video_chunk_iterator():
    for item in video_iterator():
        ...
        id = item["id"]
        audio_fn = os.path.join(STORAGE_DIR, '{}/{}.wav'.format(id, id))
        video_fn = os.path.join(STORAGE_DIR, '{}/{}.mp4'.format(id, id))
        ...
        if frames is None:
            print("Couldn't extract frames from video {}".format(item['id']))
            continue
...
def grouped_iterator(iterator, group_size, max_items=100, pop_from_front_prob=0.8):
    ...
    def _pop():
        ...
        print("Popping the TOP one ({}, {})".format(k0, l0))
    ...
    for c_i, chunk in enumerate(iterator()):
        ...
        while len(buffer) > max_items:
            ...
            if x is not None:
                ...
                print("WARNING: BUFFER with max_items={} MIGHT NOT BE BIG ENOUGH".format(max_items))
    while len(buffer) > 0:
        x = _pop()
        if x is not None:
            print("3333")
...
def _allpairs_cosine_similarity(x):
    ...
    denominator_elems = torch.sqrt(torch.diag(pairwise_numerator)) # n
    denominator = denominator_elems[None] * denominator_elems[:, None]
    cosine_sim = pairwise_numerator / denominator
    return cosine_sim

def text_iterator(num_seqs = 1, text_len=512):
    """
    ...
    file_id = random.randint(1, 16410)#args.fold % 16410
    zst_fn = os.path.join("/mnt2/user15/merlot_r/merlot_reserve_backup/data/thepile", 'fold{0:05d}of16410.jsonl.zst'.format(file_id))

    #NUM_SKIPEVERY = args.num_folds // 16410 + 1

    #skip_every = (args.fold // 16410) % NUM_SKIPEVERY
    #blob = bucket.blob(f'thepile/thepile/fold{file_id:05d}of16410.jsonl.zst')
            ...
            if random.random() > 0.9:
                with torch.no_grad():
                    ...
                    if avg_cosine_sim > args.max_acs:
                        print("breaking ACS is {} on {}".format(avg_cosine_sim, youtube_id))
                        continue
                    ...
                    if num_coco_objects_expectation < args.min_nco:
                        print("breaking NCO is {} on {}".format(num_coco_objects_expectation, youtube_id))
                        continue
        yield chunk_group
...
with GCSTFRecordWriter(train_file, buffer_size=10000, auto_close=False) as train_writer:
    for chunks, txt in tqdm(zip(buffered_chunk_iterator(), text_iterator(num_seqs=args.num_text_seqs, text_len=args.text_len))):
        ...
        if num_written % 10 == 0:
            ...
            print('num_videos={} is processed '.format(num_videos))
            # print(
            #     f"Wrote {num_written} in {te:.3f}; num_videos={num_videos}, num_tokens={tokens_sum},\
            #      max_tokens_chunk={tokens_max}, tokens_mean={tokens_mean:.2f}, tokens_95perc={tokens_95perc}",
            #     flush=True)
    ...
    print("Wrote {} ; {} videos".format(num_written, num_videos))
    train_writer.close()

total_video_sets = set()
print("Check IDS")
with tf.io.gfile.GFile(args.ids_fn, 'r') as f:
    reader = csv.DictReader(f)
    for i, row in enumerate(reader):
        #if i % args.num_folds == args.fold:
        total_video_sets.add(row['video_id'])

total_video_sets = total_video_sets - video_set
print("num videos to change")
print(len(total_video_sets))
with open(args.ids_fn_o, 'w') as f:
    fieldnames = ['video_id']
    writer = csv.DictWriter(f, fieldnames=fieldnames)
    writer.writeheader()
    for x in total_video_sets:
