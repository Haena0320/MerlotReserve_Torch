...
import time
from pretrain.data_utils import * #resize_and_pad, uniform_random_select, random_categorical_without_replacement, sample_bernoulli, batch_index_iterator, sample_bernoullis, cumulative_maximum_int, pad_to_fixed_size , encode_string
from mreserve.utils.lowercase_encoder import get_encoder, START, END, PADDING, MASK, AUDIOSPAN, LTOVPOOL, MASKAUDIO
import math
...
def increase_textmask(tokens_ragged, *, mask_idx, tok_centroids_vals, audio_start_end, right_to_left, delta_thresh=0.1):
    ...
    row_lengths = [len(t) for t in tok_centroids_expanded]
    if right_to_left:
        t_out_right = [tok_centroids_expanded[i] for i in (mask_idx+2).numpy()]
        ...
        t_out_left = [tok_centroids_expanded[i] for i in mask_idx.numpy()]
...
class MerlotDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(num_text2audio_seqs):
            ...
            spans_all.append(pad_to_fixed_size(spans, PADDING, config["text_span_length"]))
        ...
        for i in range(config['num_text_seqs']):
            ...
            spans_all.append(pad_to_fixed_size(spans_i, PADDING, config["text_span_length"]))
        features["text_spans"] = torch.cat(spans_all, 0)
        ...
        features['video_src_index'] = torch.stack([torch.tensor(x['video_src_index']) for x in segment_list])
...
class MerlotLoader(DataLoader):
    @classmethod
    def from_dataset(cls, data, batch_size=4, num_workers=3, num_gpus=1, is_training=True, **kwargs):
        ...
        collate_fn = handle_batch,
        drop_last=is_training,
        pin_memory=False,
        **kwargs)
        return loader

def input_builder(config, fns=None, num_workers=2, batch_size=4,num_devices=1, is_training=True):
    all_data = []
    for fn in fns:
        data = MerlotDataset(config, fn, num_devices=num_devices)
        all_data.append(data)
    all_dataset = torch.utils.data.ConcatDataset(all_data)
    loader = MerlotLoader.from_dataset(all_dataset, batch_size, num_workers, num_devices, is_training)
    return loader

def handle_batch(batched_tensor, num_devices=None):
    batch = dict()
    logging.info("BEFORE HANDLING BATCH")

    def stack_tensor(key=None, data_list=None):
        return torch.stack([data[key] for data in data_list])

    get_dummies = functools.partial(stack_tensor, data_list=batched_tensor)

    for k, v in batched_tensor[0].items():
        if k not in ["youtube_id", "meta"]:
            batch[k] = get_dummies(k) 

    batch_size, num_segments, hw, ppthree_ = batch['images'].shape

    if num_devices is not None:
        assert num_devices <= batch_size
        assert batch_size % num_devices == 0
        shape_prefix = [num_devices, batch_size // num_devices]
        logging.info("{} devices: shape prefix is {}".format(num_devices, shape_prefix))
    else:
        logging.info("No devices, batch size is just {}".format(batch_size))
        shape_prefix = [batch_size]

    batch["images"] = batch["images"].reshape(shape_prefix + [num_segments * hw, ppthree_])
